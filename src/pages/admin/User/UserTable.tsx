...
function UserTable(props: { intl: IntlFormatters }) {
  ...
  useEffect(() => {
    if (users) {
      ...
      const userArray = memberships
        .map((userItems: UserMembership) => {
          const school_memberships = _get(
            userItems,
            "user.school_memberships",
            [],
          );
          const schools = school_memberships.map((e: Schools) => e.school);
        ...
        .filter((e: UserMembership) => e.user_id !== userId);
      ...
      const rolSchool = roles.filter(
        (item: Role) => !constantValues.rolesNotAllowed.includes(item.role_name),
      );
      ...
      <MaterialTable
        ...
        columns={[
          ...
          },
          {
            ...
            render: (rowData) => (
              <span>{moment(rowData.birth_year_month).format("YYYY-MM")}</span>
            ),
            editComponent: (props: EditComponentProps<any>) => {
              const value = !props.value ? null : moment(props.value);
                ...
                <FormControl fullWidth error={hasError}>
                  <MuiPickersUtilsProvider utils={DateFnsUtils}>
                    <KeyboardDatePicker
                      ...
                      TextFieldComponent={({ InputProps, value }) => (
                        <TextField
          ...
          },
          {
            ...
            editComponent: (props: EditComponentProps<any>) => {
              const value = props.value
                ? props.value.map((rolItem: Role) => rolItem.role_id || rolItem)
                : [""];
                  ...
                  <Select
                    ...
                    onChange={(
                      ...
                      const valueSelected: string[] = event.target.value.filter(
                        (rolItemSelected: string) => rolItemSelected !== "",
                      );
              ...
              return (
                <span>
                  {rowData.roles.map((role: Role) => {
                    return <div key={role.role_id}>{role.role_name}</div>;
                  })}
                </span>
              );
            },
          },
          {
            ...
            editComponent: (props: EditComponentProps<any>) => {
              const value = props.value
                ? props.value.map(
                    (school: School) => school.school_id || school,
                  )
                : [""];
                ...
                <FormControl fullWidth>
                  <Select
                    ...
                    onChange={(
                      ...
                      if (
                        valueSelected.includes(constantValues.allSchoolsValue)
                      ) {
                        if (
                          ...
                          valueSelected = valueSelected.filter(
                            (item: string) =>
                              item === constantValues.allSchoolsValue,
                          );
                        } else {
                          valueSelected = valueSelected.filter(
                            (item: string) =>
                      ...
                      if (
                        valueSelected.includes(constantValues.noSchoolsValue)
                      ) {
                        if (
                          valueSelected[valueSelected.length - 1] ===
                          ...
                          valueSelected = valueSelected.filter(
                            (item: string) =>
                              item === constantValues.noSchoolsValue,
                          );
                        } else {
                          valueSelected = valueSelected.filter(
                            (item: string) =>
                  ...
                  >
                    ...
                    <MenuItem
                      ...
                      value={constantValues.allSchoolsValue}
                    >
                      {constantValues.allSchoolsValue}
                    </MenuItem>
                    <MenuItem
                      key={constantValues.noSchoolsValue}
                      value={constantValues.noSchoolsValue}
                    >
                      {constantValues.noSchoolsValue}
                    </MenuItem>
                  </Select>
          ...
          },
          {
            ...
            render: (rowData) => (
              <span className={classes.date}>{rowData.join_timestamp}</span>
            ),
          },
        ]}
        localization={{
